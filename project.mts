  ...
  private setup(setup: SetupContext) {
    try {
      ...
      this.projectContext = {
        ...
        binaryDir: presets.configurePresets[0].binaryDir.replace(
          /\$\{(.*?)\}/g,
          (_: string, p1: string) => eval(p1),
        ),
        installDir: presets.configurePresets[0].installDir.replace(
          /\$\{(.*?)\}/g,
          (_: string, p1: string) => eval(p1),
        ),
        buildType: presets.configurePresets.find(
          (item) => item.name == setup.cmakePreset.selectedPreset,
        ).cacheVariables.CMAKE_BUILD_TYPE,
      };
    } catch (e) {
      throw new Error(
        "Failed to parser cmake presets, please make sure the exists of this preset or the format of the preset is correct",
      );
    }
    this.stateMachine = {
...
class Excutor {
  ...
  private cmakeOptionsTransform() {
    ...
    for (const [key, value] of Object.entries(this.context.cmakeOptions)) {
      if (typeof value === "boolean")
        cmakeOptions.push(
          `-D${this.context.projectContext.projectName}_${this.camelToSnake(key).toUpperCase()}:BOOL=${value ? "ON" : "OFF"}`,
        );
      else
        cmakeOptions.push(
          `-D${this.camelToSnake(key).toUpperCase()}:STRING=${value}`,
        );
    }
    return cmakeOptions;
  }
  ...
  async cmakeConfigure() {
    ...
    if (process.platform === "win32") {
      ...
      const cmakeConfigreCmd =
        `"cmake -S . --preset=${this.context.cmakePreset} ${this.cmakeOptionsTransform().join(" ")}"`.trim();
      ...
      await this.excutecheckExitCode(
        cmakeConfigreCmd,
        "Cmake configure failed",
      );
      await this.excutecheckExitCode(
        symlinkCompileCommandsCmd,
        "Unable to create compile_commands.json",
      );
    } else if (process.platform === "linux") {
      const cmakeConfigreCmd =
        `cmake -S . --preset=${this.context.cmakePreset} ${this.cmakeOptionsTransform().join(" ")}`.trim();
      ...
      throw new Error(
        "Unsupported platform or compiler, Only support msvc on windows and gcc on linux",
      );
    }
  }
  ...
  async cmakeBuild() {
    ...
    refreshEnv(
      `${sourceCommandPrefix}${this.context.projectContext.binaryDir}/conan/build/${this.context.projectContext.buildType}/generators/conanbuild.${scriptPostfix}`,
    );
    refreshEnv(
      `${sourceCommandPrefix}${this.context.projectContext.binaryDir}/conan/build/${this.context.projectContext.buildType}/generators/conanrun.${scriptPostfix}`,
    );
    if (process.platform === "win32") {
      setupMSVCDevCmd({
        arch: "x64",
      ...
      const cmakeBuildCmd =
        `"cmake --build ${this.context.projectContext.binaryDir} --target ${this.context.projectContext.buildTarget.join(" ")}"`.trim();
  ...
  async runTarget() {
    this.context.projectContext.buildTarget =
      this.context.projectContext.launchTarget;
    ...
    if (process.platform === "win32") {
      ...
      const runTargetCmd =
        `"${this.context.projectContext.binaryDir}/bin/${this.context.projectContext.launchTarget[0]}.exe ${this.context.projectContext.launchArgs.join(" ")}"`.trim();
      await this.excutecheckExitCode(runTargetCmd, "Run target failed");
    } else if (process.platform === "linux") {
      const runTargetCmd =
        `${this.context.projectContext.binaryDir}/bin/${this.context.projectContext.launchTarget[0]} ${this.context.projectContext.launchArgs.join(" ")}`.trim();
      await this.excutecheckExitCode(runTargetCmd, "Run target failed");
    } else {
      throw new Error(
        "Unsupported platform or compiler, Only support msvc on windows and gcc on linux",
  ...
  async runTest() {
    if (
      this.context.stateMachine.currentState < State.Build ||
      this.context.projectContext.buildTarget[0] != "all"
    ) {
    ...
    if (process.platform === "win32") {
      ...
      const runTestCommand =
        `"ctest --preset ${this.context.cmakePreset} ${this.context.projectContext.testArgs.join(" ")}"`.trim();
  ...
  async runCov() {
    ...
    if (process.platform === "win32") {
      ...
      const runCovCmd =
        `"OpenCppCoverage.exe --export_type cobertura:out/coverage/coverage.xml --export_type html:out/coverage --cover_children -- ctest --preset ${this.context.cmakePreset} ${this.context.projectContext.testArgs.join(" ")}"`.trim();
...
function showHelp() {
  ...
  console.log(
    chalk.green(
      " tsx project.mts build [target_name]                 ---build the target [target_name]",
    ),
  );
  console.log("\n");
  ...
  console.log(
    chalk.green(
      " tsx project.mts run [target_name]                   ---run the target [target]",
    ),
  );
  console.log(
    chalk.green(
      " tsx project.mts run [target_name] [-- target_args]  ---run the target [target_name] with target_args",
    ),
  );
  console.log(
    chalk.green(
      " tsx project.mts run                                 ---rerun last specified target",
    ),
  );
  console.log(
    chalk.green(
      " tsx project.mts run!                                ---rerun last specified target with last args",
    ),
  );
  console.log("\n");
  ...
  console.log(
    chalk.green(
      " tsx project.mts test [test_name]                    ---run the test [test_name]",
    ),
  );
  console.log("\n");
  ...
  console.log(
    chalk.hex("0xa9cc00")(
      "Usage: tsx project.mts <action>[!] [target] [-- args]",
    ),
  );
  console.log(
    chalk.hex("0xa9cc00")(
      "action: config | clean | build | run | test | install | pack",
    ),
  );
  console.log(chalk.hex("0xa9cc00")("[!] : reuse last args"));
  console.log(
    chalk.hex("0xa9cc00")("target: the target to execute the action"),
  );
  console.log(
    chalk.hex("0xa9cc00")("args: the arguments to pass to the target"),
  );
}
...
async function main() {
  ...
  if (cmdsNotFound.length > 0) {
    console.log(
      chalk.yellowBright(
        `Some commands not found in path:${cmdsNotFound} ,Tring reload the environment...`,
      ),
    );
    if (process.platform === "win32") {
      refreshEnv("refreshenv");
    } else if (process.platform === "linux") {
      if (fs.existsSync(`${process.env.HOME}/.profile`)) {
        refreshEnv("source ~/.profile");
      } else if (fs.existsSync(`${process.env.HOME}/.bashrc`)) {
        refreshEnv("source ~/.bashrc");
      } else {
        console.log(chalk.redBright("Neither ~/.profile nor ~/.bashrc found."));
      }
    }
  }
  ...
  if (myArgv._[0] == "setup") {
    ...
    const cmakeOptions = Object.entries(myArgv)
      .filter(([key, _]) => {
      ...
      .map(([key, value]) => {
        if (typeof value === "string") {
          if (value.toUpperCase() == "ON") {
            ...
            throw new Error(
              `Error parameter type for ${key}, use 'ON' or 'OFF' for cmakeOptions`,
            );
          }
        } else {
          throw new Error(
  ...
  if (myArgv._[0] == "cov") {
    ...
    if (
      ...
      console.log(
        chalk.yellowBright(
          "Coverage is not enabled, trying to enable it and build again...",
        ),
      );
    ...
    if (
      ...
      console.log(
        chalk.yellowBright(
          "Last time run cmake configure with COVERAGE flag ON temporarily, disable it and reconfigure the project...",
        ),
      );
    ...
    switch (myArgv._[0]) {
      ...
      case "config":
        ...
        const envList = getEnvDiff(
          `${sourceCommandPrefix}${context.projectContext.binaryDir}/conan/build/${context.projectContext.buildType}/generators/conanrun.${scriptPostfix}`,
        );
        replaceJsonNode(
          ".vscode/launch.json",
          "configurations",
      ...
      case "build":
        ...
        if (myArgv._.length > 1) {
          console.log(
            chalk.greenBright("Building target:", myArgv._.slice(1).join(",")),
          );
      ...
      case "run":
        ...
        if (myArgv._.length > 1) {
          ...
          console.log(
            chalk.greenBright("Runing target:", targetContext.target.join(" ")),
          );
        } else {
        ...
        replaceJsonNode(
          ...
          ["type", "lldb"],
          "program",
          `${context.projectContext.binaryDir}/bin/${targetContext.target}`,
        );
        replaceJsonNode(
          ".vscode/launch.json",
          "configurations",
